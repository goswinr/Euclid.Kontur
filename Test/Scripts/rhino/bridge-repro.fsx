...
#r "nuget: Klip, 4.0.0"
#r "D:/Git/_Euclid_/Euclid.Kontur/Src/bin/Release/net6.0/Euclid.Kontur.dll"
open Fesher
open Euclid
open Klip
...
let unionKlip tol (ps:Klip.Paths64<unit>) =
...
let unionKontur tol (ps: Euclid.Kontur) =
    ps |> Euclid.Kontur.simplifyWith tol


let unionClipper (precision:int) (ps:Clipper2Lib.PathsD) =
...
let areaOkKlip (a:Klip.Paths64<_>)  (b:Klip.Paths64<_>) =
    let aa = a |> Seq.sumBy Klip.Path64.absArea
    let bb = b |> Seq.sumBy Klip.Path64.absArea
    abs(aa-bb) < (max aa bb) * 0.01 // within 1 %

let areaOkKontour (a:Euclid.Kontur)  (b:Euclid.Kontur) =
    let aa = a.SignedArea
    let bb = b.SignedArea
    abs(aa-bb) < (max aa bb) * 0.01 // within 1 %
...
let k = ref 0
let drawKontur lay (ps:Euclid.Kontur) =
    incr k
    if !k < 10 then 
        for p in ps.Paths do
            p
            |> Polyline2D.toRhPolylineCurve
            |> rs.Ot.AddCurve
            |> rs.setLayer $"draw::{lay}"        
...
let run shift rot scale =
    ...
    let pathKontur =
        Euclid.Kontur.create (input, Euclid.FillRule.NonZero)

    // let res = unionKlip (scale* 1e-5) pathK
    let res = unionKontur (scale* 1e-5) pathKontur
    drawKontur $"shift {shift} rot {rot}" res
    drawKontur $"input" pathKontur

    if res.PathCount = 1 && res.Paths[0].PointCount = 12 && areaOkKontour pathKontur res then
        incr OKk
        // Printfn.green $"SUCCESS,scale {scale} shift:{shift},  rot {rot}"

    elif res.PathCount = 0 then
        incr NotOKk
        Printfn.orange $"ERROR, scale {scale} shift:{shift}, rot {rot}, empty path"
    else
        incr NotOKk
        Printfn.red $"ERROR,scale {scale} shift:{shift}, rot {rot}: {res.PathCount} paths, first has {res.Paths[0].PointCount} points, area ok: {areaOkKontour pathKontur res}"

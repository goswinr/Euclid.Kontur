...
let tests =
    testList ("Shape", [
        ...
        test ("open or too short paths fail", fun _ ->
            ...
            assertThat (fun () -> Shape.ofPolyline openPath |> ignore) (tag "open path" >> throws)
            let short = Polyline2D 3
            short.AddXY (0.0, 0.0)
            short.AddXY (1.0, 0.0)
            short.AddXY (0.0, 0.0)
            assertThat (fun () -> Shape.create ([short], FillRule.NonZero) |> ignore) (tag "two distinct points" >> throws)
        )
        ...
        test ("bounding rectangle spans all paths", fun _ ->
            ...
            assertThat
                (fun () -> (Shape.empty FillRule.NonZero).BoundingRectangle |> ignore)
        ...
        test ("Positive and Negative depend on orientation", fun _ ->
            ...
            let cw = ccw.Reverse ()
            let pt = Pt (5.0, 5.0)
            assertThat ((Shape.ofPolyline (ccw, FillRule.Positive)).Contains pt) (tag "ccw positive" >> isTrue)
            assertThat ((Shape.ofPolyline (ccw, FillRule.Negative)).Contains pt) (tag "ccw negative" >> isFalse)
            assertThat ((Shape.ofPolyline (cw,  FillRule.Positive)).Contains pt) (tag "cw positive" >> isFalse)
            assertThat ((Shape.ofPolyline (cw,  FillRule.Negative)).Contains pt) (tag "cw negative" >> isTrue)
        )
        ...
        test ("SignedArea sums the paths", fun _ ->
            ...
            assertThat (Shape.empty FillRule.NonZero).SignedArea (tag "empty" >> isEqualTo 0.0)
        )
    ])

...
let private area (s: Kontur) = s.SignedArea

/// Checks that a result Kontur is well formed: closed paths, at least 3 distinct points, Positive rule.
let private checkResult (r: Kontur) =
...
let private oracle (rand: Random) (subject: Kontur) (clip: Kontur) (op: ClipType) (result: Kontur) =
    checkResult result
...
let private checkAll (rand: Random) (engine: KonturEngine) (subject: Kontur) (clip: Kontur) =
...
let tests =
    testList ("Engine", [
        ...
        test ("two overlapping squares", fun _ ->
            ...
            let inter = Kontur.intersection a b
            ...
            checkAll (Random 1) (KonturEngine 1e-6) a b
        )
        ...
        test ("disjoint squares", fun _ ->
            ...
            assertThat (Kontur.intersection a b).IsEmpty (tag "intersection is empty" >> isTrue)
            assertThat (area (Kontur.difference a b)) (tag "difference is a" >> isCloseTo Accuracy.high 1.0)
            assertThat (area (Kontur.xor a b)) (tag "xor is both" >> isCloseTo Accuracy.high 2.0)
        )
        ...
        test ("square minus nested square gives a hole", fun _ ->
            ...
            assertThat (area (Kontur.union a b)) (tag "union is the outer" >> isCloseTo Accuracy.high 100.0)
            assertThat (area (Kontur.intersection a b)) (tag "intersection is the inner" >> isCloseTo Accuracy.high 16.0)
        )
        ...
        test ("identical squares", fun _ ->
            ...
            assertThat (Kontur.difference a b).IsEmpty (tag "difference is empty" >> isTrue)
            assertThat (Kontur.xor a b).IsEmpty (tag "xor is empty" >> isTrue)
        )
        ...
        test ("squares sharing an edge", fun _ ->
            ...
            assertThat (Kontur.intersection a b).IsEmpty (tag "no overlap" >> isTrue)
            assertThat (area (Kontur.difference a b)) (tag "difference is a" >> isCloseTo Accuracy.high 100.0)
        )
        ...
        test ("an even odd glyph unioned with a non zero outline in one pass", fun _ ->
            ...
            let solid = Kontur.create (glyph.Paths, FillRule.NonZero)
            assertThat (area (Kontur.union solid bar)) (tag "no hole under non zero" >> isCloseTo Accuracy.high (100.0 + 100.0 - 12.0))
        )
        ...
        test ("a vertex within tolerance of an edge snaps onto it", fun _ ->
            ...
            assertThat (Kontur.intersection a b).IsEmpty (tag "no area in common" >> isTrue)
        ...
        test ("empty shapes", fun _ ->
            ...
            assertThat (Kontur.intersection a e).IsEmpty (tag "a intersect empty" >> isTrue)
            assertThat (Kontur.difference e a).IsEmpty (tag "empty minus a" >> isTrue)
            assertThat (area (Kontur.difference a e)) (tag "a minus empty" >> isCloseTo Accuracy.high 100.0)
            assertThat (Kontur.union e e).IsEmpty (tag "empty union empty" >> isTrue)
        )
        ...
        test ("many paths per shape against the point oracle", fun _ ->
            ...
            let engine = KonturEngine 1e-6
            for _ in 1 .. 10 do
                let a = Kontur.create ([ for _ in 1 .. 4 do randomStar rand (rand.NextDouble () * 20.0) (rand.NextDouble () * 20.0) 6.0 8 true ], FillRule.NonZero)
                let b = Kontur.create ([ for _ in 1 .. 4 do randomTangle rand (rand.NextDouble () * 20.0) (rand.NextDouble () * 20.0) 6.0 6 ], FillRule.EvenOdd)
                checkAll rand engine a b
        )
        ...
        test ("a segment within tolerance of the seed vertex does not flip the component", fun _ ->
            ...
            let check (dx: float) (dy: float) (ux: float) (uy: float) =
                ...
                let r = Kontur.simplify (Kontur.create ([ tri; quad ], FillRule.NonZero))
                checkResult r
